...
const GripIcon   = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" opacity="0.5"><circle cx="8" cy="6" r="1.5"/><circle cx="16" cy="6" r="1.5"/><circle cx="8" cy="12" r="1.5"/><circle cx="16" cy="12" r="1.5"/><circle cx="8" cy="18" r="1.5"/><circle cx="16" cy="18" r="1.5"/></svg>;
...
const RefreshIcon= () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>;
  ...
  return (
    <div className="pf">
      <div className="pf__head">
        <span className="pf__label">{isNew?"Add New Product":"Edit Product"}</span>
        <button className="pf__close" onClick={onCancel}><CloseIcon/></button>
      </div>
      <div className="pf__body">
        ...
        <div className="pf__field pf__field--full">
          ...
          {!preview ? (
            <div className={`pf__upload-zone${dragOver?" pf__upload-zone--over":""}`}
              ...
              {compressing
                ? <><div className="pf__compress-spinner"/><span className="pf__upload-title">Processing…</span></>
                : <><UploadIcon/><span className="pf__upload-title">Click or drag & drop</span><span className="pf__upload-hint">JPG · PNG · WEBP — auto-compressed</span></>}
            </div>
          ) : (
...
function ProductRow({ product, index, onEdit, onDelete, onMoveUp, onMoveDown, isFirst, isLast }) {
  ...
  return (
    <div className="pr">
      {/* Row 1: image + info + order */}
      <div className="pr__top">
        <div className="pr__drag"><GripIcon/></div>
        <div className="pr__img-wrap">
          {product.img
            ? <img src={product.img} alt={product.title} className="pr__img"/>
            : <div className="pr__img-ph"><ImageIcon/></div>}
        ...
        <div className="pr__info">
          ...
          <div className="pr__title">{product.title}</div>
          <div className="pr__order">
      ...
      <div className="pr__actions">
        ...
        {confirmDelete ? (
          <div className="pr__confirm">
            <span>Delete?</span>
            <button className="pr__confirm-yes" onClick={onDelete}>Yes</button>
            <button className="pr__confirm-no" onClick={()=>setConfirmDelete(false)}>No</button>
          </div>
        ) : (
...
function InquiriesTab() {
  ...
  const fetchInquiries = async () => {
    setLoading(true);
    try { const res = await fetch('/api/inquiries'); setInquiries(await res.json()); }
    catch(e) {}
  ...
  if (loading) return <div className="admin__loading"><div className="admin__spinner"/><span>Loading inquiries…</span></div>;
  ...
  return (
    <div className="inq">
      <div className="inq__header">
        ...
        <button className="inq__refresh" onClick={fetchInquiries}><RefreshIcon/> Refresh</button>
      </div>

      <div className="inq__filters">
        {[{key:"all",label:"All"},{key:"new",label:"New"},{key:"contacted",label:"Contacted"},{key:"done",label:"Done"}].map(f=>(
          <button key={f.key} className={`inq__filter${filter===f.key?" inq__filter--active":""}`} onClick={()=>setFilter(f.key)}>
            {f.label}<span className="inq__filter-count">{counts[f.key]}</span>
          </button>
        ))}
      </div>
                ...
                <div className="inq-card__body">
                  ...
                  <div className="inq-card__top">
                    ...
                    <div className="inq-card__contact">
                      ...
                      <a href={`tel:${inq.phone}`} className="inq-card__phone"><PhoneIcon/>{inq.phone}</a>
                      {inq.email && <a href={`mailto:${inq.email}`} className="inq-card__email"><MailIcon/>{inq.email}</a>}
                    </div>
                  ...
                  {/* Row 3: message */}
                  {inq.message && <p className="inq-card__message">"{inq.message}"</p>}
                  {/* Row 4: time */}
                  ...
                  <div className="inq-card__actions">
                    <a href={`tel:${inq.phone}`} className="inq-act inq-act--call"><PhoneIcon/> Call</a>
                    <a href={`https://wa.me/${waNumber}`} target="_blank" rel="noreferrer" className="inq-act inq-act--wa"><WAIcon/> WhatsApp</a>
                    {inq.status==='new' && <button className="inq-act inq-act--progress" onClick={()=>updateStatus(inq.id,'contacted')}><CheckIcon/> Contacted</button>}
                    {inq.status==='contacted' && <button className="inq-act inq-act--done" onClick={()=>updateStatus(inq.id,'done')}><CheckIcon/> Mark Done</button>}
                    <button className="inq-act inq-act--del" onClick={()=>deleteInquiry(inq.id)} title="Delete"><TrashIcon/></button>
                  </div>
                </div>
              </div>
            );
    ...
    <div className="admin">
      <header className="admin__hdr">
        ...
        <div className="admin__hdr-right">
          <a href="/" className="admin__preview-btn" target="_blank" rel="noreferrer">Preview ↗</a>
        </div>
      </header>
            ...
            <div className="admin__filter-tabs">
              {["All",...SECTIONS].map(s=>(
                <button key={s} className={`admin__filter-tab${filter===s?" admin__filter-tab--active":""}`} onClick={()=>setFilter(s)}>
                  {s}<span className="admin__filter-count">{s==="All"?products.length:products.filter(p=>p.section===s).length}</span>
                </button>
              ))}
            </div>
